...
| 模型 | 厂商 | 说明 |
|---|---|---|
| `claude-opus-5` / `claude-sonnet-5` / `claude-haiku-4-5` | Anthropic | Messages 协议 |
| `glm-4.6v-flash` | 智谱 | 免费视觉 |
| `glm-4.1v-thinking-flash` | 智谱 | 免费视觉，带思考 |
| `glm-4.1v-thinking-flashx` | 智谱 | 付费，更快 |

两家协议不同——Anthropic 走 Messages，智谱走 OpenAI 兼容的
`https://open.bigmodel.cn/api/paas/v4/chat/completions`——差异封在 `AgentClient` 里，
上层四个任务感知不到。智谱的 `temperature` 区间不含 0，传 0 会被拒，客户端自动贴着下界走。

识别默认挑免费模型：录入频率是系统生命线，识别不该按次心疼。
标注那一档不建议为省钱降配，它直接决定聚合是否可信。

模型 ID 会随厂商更新漂移，所以设置页留了自定义 ID 输入框并可指定厂商，
换 ID 不需要等发版。

## 联网范围

识别、标注、报告生成、树生成与维护需要 API Key（设置页按厂商分别填写）。
