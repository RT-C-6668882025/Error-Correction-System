...
**原题的增删由人决定，判重由程序给依据。** 入库时按 uid（卷名#题号_空序）去重，
挡得住同一份卷子重复提交，挡不住换个卷名再拍一次、或导入一份别处导出的备份——
uid 不一样，同一道题就在原题页里躺两条，分析各跑一次，汇总时还会被当成两次证据。
`core/dup/Duplicates` 按「题干 + 空序 + 答案」判重（归一到写法差别：大小写、空白、
下划线长度、标点；空的位置保留，否则同一句话的三个空会被并成一条），
每一簇挑一条留下——已分析的优先，其次有答案的，再其次录得早的。
它只负责选中并把「留哪条」标在卡片上，删除永远是人点的，并且删之前自动存一份备份。

**两档可以是不同厂商。** 识别天天跑要便宜，判断决定分析质量要强，本来就很难是同一家。
存 Key 只补上还空着的那一档（原来是无条件接管两档，于是给第二个厂商填一次 Key，
「识别走智谱、判断走 Anthropic」就被悄悄拆成两档同一家）；要改哪一档，
用「只设识别档 / 只设判断档」，想两档都换才点「两档都用它」。

**清单接口不是每家都有。** 拉清单只是省得手抄 ID：拉不到照样能用内置候选和手填。
地址会试两个（版本段下的 /models 与根下的 /v1/models），报错里带上试过的地址与状态码，
并明说「跟 Key 对不对无关」——404 是这个厂商没开放清单接口，不是配置错了。

**分析跑哪些由选择决定。** 原题页长按进多选，选中的一起分析或一起删；
一条也是它，三十条也是它。原来只有一个「分析全部待分析」，重录一份卷子之后
想只跑新的那几条，只能一条条点开。
